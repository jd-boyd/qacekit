...
AceKit::AceKit(QWidget *parent) : QWidget(parent) {
  ...
  wv->page()->settings()->setAttribute(QWebSettings::DeveloperExtrasEnabled,
                                       true);
  attachObject();
...
void AceKit::setContent(QString content) {
  ...
  frame->evaluateJavaScript(
      "window.qacekit_editor.setValue(AceKit.transfer_content);");
}
...
void AceKit::setTheme(QString theme) {
  ...
  frame->evaluateJavaScript(
      "window.qacekit_editor.setTheme(AceKit.transfer_content);");
}

void AceKit::setMode(QString mode) {
  transfer_content = mode;
  frame->evaluateJavaScript(
      "window.qacekit_editor.getSession().setMode(AceKit.transfer_content);");
}
